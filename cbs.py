...
def detect_collision(pathA, pathB, startTimeA, startTimeB, goalA, goalB):
    ##############################
    ...
    for t in range(length):
        isValid = t >= startTimeA and t >= startTimeB
        if not isValid:
            continue
        # check for vertex collision
        pos1 = get_location(path1, t)
        pos2 = get_location(path2, t)
        if pos1 == goalA or pos2 == goalB:
            continue
        if pos1 == pos2:
...
def detect_collisions(paths, startTimes, goals):
    ##############################
    ...
    for i in range(len(paths)):
        for j in range(i + 1, len(paths)):
            coll_data = detect_collision(paths[i], paths[j], startTimes[i], startTimes[j], goals[i], goals[j])
...
class CBSSolver(object):
    ...
    def find_solution(self, disjoint=False):
        ...
        for i in range(self.num_of_agents):  # Find initial path for each agent
            ...
            root['startTimes'].append(self.starts[i][2])
            root['goals'].append(self.goals[i])

        root['cost'] = get_sum_of_cost(root['paths'])
        root['collisions'] = detect_collisions(root['paths'], root['startTimes'], root['goals'])
        self.push_node(root)
        # Task 3.1: Testing
        if DEBUG:
        ...
        while self.open_list and timer.time() - self.start_time < self.max_time:
            ...
            for c in constraints:
                ...
                if path:
                    ...
                    if(not skip_node):
                        q['collisions'] = detect_collisions(q['paths'],q['startTimes'],q['goals'])
