...
def is_constrained(curr_loc, next_loc, next_time, constraint_table):
    ...
    if next_time in constraint_table:
        ...
        for c in constraints:
            if [next_loc] == c['loc'] and c['final'] and next_loc != (0,0):
                return True
    return False
...
def printConstraintTable(c_table):
    for t, c in c_table.items():
        print(f"Time: {t}")
        for constraint in c:
            print(f"\t{constraint}")
        print()
...
def a_star(my_map, start, goal_loc, h_values, agent, constraints):
    ...
    while len(open_list) > 0:
        ...
        for direction in range(5):
            ...
            if direction < 4 and curr['time'] > startTime:
